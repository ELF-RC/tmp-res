...
class EmptyImage(Image):
    ...
    def TotalSha1(self, include_clobbered_blocks=False):
        ...
        if not (self.clobbered_blocks.size() == 0):
            raise AssertionError(f"Assertion failed: 'self.clobbered_blocks.size() == 0'")
...
class DataImage(Image):
    ...
    def __init__(self, data, trim=False, pad=False):
        ...
        if not (not (trim and pad)):
            raise AssertionError(f"Assertion failed: 'not (trim and pad)'")
        ...
        if not (len(self.data) % self.blocksize == 0):
            raise AssertionError(f"Assertion failed: 'len(self.data) % self.blocksize == 0'")
        ...
        if not (zero_blocks or nonzero_blocks or clobbered_blocks):
            raise AssertionError(f"Assertion failed: 'zero_blocks or nonzero_blocks or clobbered_blocks'")
...
class Transfer(object):
    ...
    def ConvertToNew(self):
        if not (self.style != "new"):
            raise AssertionError(f"Assertion failed: 'self.style != 'new''")
        self.use_stash = []
        self.style = "new"
...
class BlockImageDiff(object):
    def __init__(self, tgt, src=None, version=4, threads=None,
        ...
        if not (version in (1, 2, 3, 4)):
            raise AssertionError("version must be in (1, 2, 3, 4)")
        ...
        if not (tgt.blocksize == 4096):
            raise AssertionError(f"Assertion failed: 'tgt.blocksize == 4096'")
        if not (src.blocksize == 4096):
            raise AssertionError(f"Assertion failed: 'src.blocksize == 4096'")
    ...
    def WriteTransfers(self, prefix):
      ...
      might help to avoid fsync errors on some devices."""
        ...
        for xf in self.transfers:
            ...
            if self.version < 2:
                if not (not xf.stash_before):
                    raise AssertionError(f"Assertion failed: 'not xf.stash_before'")
                if not (not xf.use_stash):
                    raise AssertionError(f"Assertion failed: 'not xf.use_stash'")

            for s, sr in xf.stash_before:
                if not (s not in stashes):
                    raise AssertionError(f"Assertion failed: 's not in stashes'")
                if free_stash_ids:
            ...
            if self.version == 1:
                ...
                for s, sr in xf.use_stash:
                    ...
                    if self.version == 2:
                        ...
                        if not (sh in stashes):
                            raise AssertionError(f"Assertion failed: 'sh in stashes'")
            ...
            if xf.style == "new":
                if not (xf.tgt_ranges):
                    raise AssertionError(f"Assertion failed: 'xf.tgt_ranges'")
                out.append("%s %s\n" % (xf.style, xf.tgt_ranges.to_string_raw()))
                total += tgt_size
            elif xf.style == "move":
                if not (xf.tgt_ranges):
                    raise AssertionError(f"Assertion failed: 'xf.tgt_ranges'")
                if not (xf.src_ranges.size() == tgt_size):
                    raise AssertionError(f"Assertion failed: 'xf.src_ranges.size() == tgt_size'")
                ...
                if not (xf.tgt_ranges):
                    raise AssertionError(f"Assertion failed: 'xf.tgt_ranges'")
                if not (xf.src_ranges):
                    raise AssertionError(f"Assertion failed: 'xf.src_ranges'")
                if self.version == 1:
                ...
                if not (WriteTransfersZero(out, to_zero) == to_zero.size()):
                    raise AssertionError("WriteTransfersZero mismatch")
                total += to_zero.size()
            else:
            ...
            if self.version >= 2 and Settings.cache_size is not None:
                ...
                if not (max_stashed_blocks * self.tgt.blocksize < max_allowed):
        ...
        if self.tgt.extended.size() > 0:
            if not (WriteTransfersZero(out, self.tgt.extended) == self.tgt.extended.size()):
                raise AssertionError("WriteTransfersZero extended mismatch")
    ...
    def ReviseStashSize(self):
        ...
        for xf in self.transfers:
            ...
            if xf.style == "diff" and self.version >= 3:
                if not (xf.tgt_ranges and xf.src_ranges):
                    raise AssertionError(f"Assertion failed: 'xf.tgt_ranges and xf.src_ranges'")
                if xf.src_ranges.overlaps(xf.tgt_ranges):
            ...
            for cmd in replaced_cmds:
                ...
                for idx, sr in cmd.use_stash:
                    ...
                    if not ((idx, sr) in def_cmd.stash_before):
                        raise AssertionError("expected (idx, sr) in stash_before")
        ...
        with open(prefix + ".new.dat", "wb") as new_f:
            for xf in self.transfers:
                if xf.style == "zero":
                    ...
                    if not (False):
                        raise AssertionError("unknown style " + xf.style)
    ...
    def AssertSequenceGood(self):
        ...
        for xf in self.transfers:
            ...
            for s, e in x:
                ...
                for i in range(s, min(e, self.tgt.total_blocks)):
                    if not (touched[i] == 0):
                        raise AssertionError(f"Assertion failed: 'touched[i] == 0'")
        ...
        for s, e in self.tgt.care_map:
            for i in range(s, e):
                if not (touched[i] == 1):
                    raise AssertionError(f"Assertion failed: 'touched[i] == 1'")
    ...
    def ImproveVertexSequence(self):
        ...
        if not (len(L) == len(self.transfers)):
            raise AssertionError(f"Assertion failed: 'len(L) == len(self.transfers)'")
        ...
        for xf in self.transfers:
            ...
            for u in xf.goes_before:
                ...
                if xf.order < u.order:
                    ...
                    out_of_order += 1
                    if not (xf.src_ranges.overlaps(u.tgt_ranges)):
                        raise AssertionError(f"Assertion failed: 'xf.src_ranges.overlaps(u.tgt_ranges)'")
        ...
        for xf in self.transfers:
            for u in xf.goes_before.copy():
                ...
                if xf.order < u.order:
                    ...
                    if not (overlap):
                        raise AssertionError(f"Assertion failed: 'overlap'")
    ...
    def FindTransfers(self):
      ...
      size by 30% volantis, and 20% for angler and bullhead."""
            ...
            if tgt_ranges.size() or src_ranges.size():
                ...
                if not (tgt_ranges.size() and src_ranges.size()):
                    raise AssertionError(f"Assertion failed: 'tgt_ranges.size() and src_ranges.size()'")
                tgt_split_name = "%s-%d" % (tgt_name, pieces)
                src_split_name = "%s-%d" % (src_name, pieces)
    ...
    equals 'total')."""
        ...
        so_far = RangeSet()
        for i in seq:
            if not (not so_far.overlaps(i)):
                raise AssertionError(f"Assertion failed: 'not so_far.overlaps(i)'")
            so_far = so_far.union(i)
        if not (so_far == total):
            raise AssertionError(f"Assertion failed: 'so_far == total'")
