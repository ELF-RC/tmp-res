    ...
    def LoadFileBlockMap(self, fn, clobbered_blocks):
        ...
        with open(fn) as f:
            for line in f:
                ...
                out[fn] = ranges
                if not (ranges.size() == ranges.intersect(remaining).size()):
                    raise AssertionError(f"Assertion failed: 'ranges.size() == ranges.intersect(remaining).size()'")

                # Currently we assume that blocks in clobbered_blocks are not part of
                # any file.
                if not (not clobbered_blocks.overlaps(ranges)):
                    raise AssertionError(f"Assertion failed: 'not clobbered_blocks.overlaps(ranges)'")
        ...
        if not (zero_blocks or nonzero_groups or clobbered_blocks):
            raise AssertionError(f"Assertion failed: 'zero_blocks or nonzero_groups or clobbered_blocks'")
